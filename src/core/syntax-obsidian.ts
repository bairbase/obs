  ...
  function calloutCoreRule(state: StateCore): void {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (body.length > 0) {
        ...
        // No body content - remove the first paragraph_open / inline /
        // paragraph_close trio so the callout can still hold content
        // (or be empty → we'll insert an empty paragraph on parse).
        // The inline is at firstInlineIdx; its paragraph_open is at
        // firstInlineIdx - 1, paragraph_close at firstInlineIdx + 1.
        const paraOpen = firstInlineIdx - 1;
        const paraClose = firstInlineIdx + 1;
        if (
          tokens[paraOpen]?.type === "paragraph_open" &&
          tokens[paraClose]?.type === "paragraph_close"
        ) {
          tokens.splice(paraOpen, 3);
          closeIdx -= 3;
        } else {
