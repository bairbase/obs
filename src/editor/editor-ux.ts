...
function showContextMenu(schema: Schema, view: EditorView, event: MouseEvent) {
  ...
  menu.addItem((item) =>
    item.setTitle("Cut").setIcon("scissors")
      // execCommand is the only programmatic path that respects the
      // user's current text selection — modern Clipboard API can write
      // arbitrary text but doesn't replicate the "cut" focus semantics.
      // eslint-disable-next-line @typescript-eslint/no-deprecated
      .onClick(() => activeDocument.execCommand("cut")),
  );
  menu.addItem((item) =>
    item.setTitle("Copy").setIcon("copy")
      // eslint-disable-next-line @typescript-eslint/no-deprecated
      .onClick(() => activeDocument.execCommand("copy")),
  );
  menu.addItem((item) =>
