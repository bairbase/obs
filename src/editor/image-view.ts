...
// Preset widths for the image size menu. Values picked to match the
// rough "small / medium / large" mental model users have without
// being too prescriptive. "Full width" is a separate display mode
// (see schema's `displayMode` attr) so it round-trips through `|full`
// rather than a magic pixel number.
const SIZE_PRESETS: Array<{ id: string; label: string; width: number | null; mode: "fixed" | "full" | null }> = [
  { id: "small", label: "Small (300px)", width: 300, mode: "fixed" },
  { id: "medium", label: "Medium (500px)", width: 500, mode: "fixed" },
  { id: "large", label: "Large (800px)", width: 800, mode: "fixed" },
  { id: "full", label: "Full width", width: null, mode: "full" },
  { id: "natural", label: "Natural (no size)", width: null, mode: null },
];
...
export function imageView(app: App, getSourcePath: () => string) {
  return (
    ...
    // Right-click context menu - opens a Size submenu so users can
    // pick a preset (Small / Medium / Large / Full width / Natural)
    // without learning the drag-resize gesture. "Full width" also
    // splits the parent paragraph so the image sits in a block by
    // itself, matching the "in practice block-level" expectation.
    wrap.addEventListener("contextmenu", (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      openImageSizeMenu(view, getPos, node, ev);
    });
    ...
    return {
      ...
      update(updated) {
        ...
        // handle attachment AND the wrap classes. Easiest: rebuild
        // the NodeView so initialization runs from scratch.
        const ua = updated.attrs as { displayMode?: string | null; src?: string; alt?: string; title?: string };
        const na = node.attrs as { displayMode?: string | null; src?: string };
        ...
        if (ua.src === na.src) {
          if (!img) return true; // broken-state DOM is static
          img.alt = ua.alt ?? "";
...
//
// Right-click on an image opens this menu. Items dispatch a
// `setNodeMarkup` transaction to update the image's `width` /
// `height` / `displayMode` attrs. Picking "Full width" additionally
// splits the parent paragraph so the image is the sole content of
// its block - full-width images mixed with inline text feel
// awkward (the image visually spans a full row but PM still treats
// surrounding text as inline siblings).
function openImageSizeMenu(
  view: EditorView,
  getPos: () => number | undefined,
  node: PMNode,
  e: MouseEvent,
  ...
  const menu = new Menu();
  const currentMode = node.attrs.displayMode as string | null;
  const currentWidth = node.attrs.width as number | null;
  // Determine which preset (if any) matches the current state, so
  // we can pre-check it.
  const currentPresetId =
    currentMode === "full"
      ...
      : currentWidth == null
        ? "natural"
        : SIZE_PRESETS.find(
            (p) => p.mode === "fixed" && p.width === currentWidth,
          )?.id ?? null;

  menu.addItem((item) => {
    item.setTitle("Size");
    item.setIcon("scaling");
    const sub = item.setSubmenu();
    sub.dom?.classList.add("butter-image-size-submenu");
    for (const p of SIZE_PRESETS) {
      sub.addItem((s) => {
        s.setTitle(p.label);
        if (p.id === currentPresetId) s.setChecked(true);
        s.onClick(() => applySizePreset(view, getPos, p));
      });
    }
  });
...
function applySizePreset(
  view: EditorView,
  getPos: () => number | undefined,
  preset: { id: string; width: number | null; mode: "fixed" | "full" | null },
) {
  const pos = getPos();
  if (pos == null) return;
  ...
  const newAttrs = {
    ...node.attrs,
    width: preset.mode === "fixed" ? preset.width : null,
    height: null,
    displayMode: preset.mode === "full" ? "full" : null,
    sourceRange: null,
  };

  let tr = view.state.tr.setNodeMarkup(pos, undefined, newAttrs);

  // Full-width mode wants the image alone in its parent paragraph.
  // If the parent has other inline content (text, marks, atoms),
  // split the paragraph at the image's boundaries so the image
  // ends up as the sole child of its own paragraph.
  if (preset.mode === "full") {
    tr = isolateImageInOwnParagraph(tr, view.state.doc, pos);
  }
  view.dispatch(tr);
  view.focus();
}

// If `imagePos` points at an image inline atom inside a paragraph
// that has other inline content, split the paragraph so the image
// is alone in its parent. Returns the transaction with any required
// splits applied; if the image is already the sole content, no-op.
function isolateImageInOwnParagraph(
  tr: Transaction,
  doc: PMNode,
  imagePos: number,
): Transaction {
  ...
  if (parent.type.name !== "paragraph") return tr;
  // Image is the only inline child → already isolated.
  if (parent.childCount === 1) return tr;
  // Split the paragraph at the boundaries on either side of the
  // image. Order matters: split AFTER the image first (so the
  // before-split offsets stay valid), then BEFORE.
  const imageEnd = imagePos + 1; // image is inline atom, size 1
  const imageStart = imagePos;
  const parentStart = $pos.before();
  const parentEnd = $pos.after();
  if (imageEnd < parentEnd - 1) {
    tr = tr.split(tr.mapping.map(imageEnd));
  }
  if (imageStart > parentStart + 1) {
    tr = tr.split(tr.mapping.map(imageStart));
  }
  return tr;
}
