  ...
  return new PMPlugin<MultiBlockSelection>({
    ...
    view(editorView) {
      ...
      const onPointerDown = (e: PointerEvent) => {
        ...
        // The drag-handle plugin creates its handle inside
        // `editorView.dom.parentElement` (= host). Multiple Butter
        // views = multiple multi-block-select instances, all with
        // doc-level pointerdown listeners. Without this check, every
        // instance would process every click on every view's handle,
        // making single clicks register as N clicks.
        const host = editorView.dom.parentElement;
        if (host && !host.contains(target)) {
          // Click outside this view's host - only the clear-on-
          // outside-click logic is relevant here; let it fall
          // through to the bottom of the function.
