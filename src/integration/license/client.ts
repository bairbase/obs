...
export interface SessionResponse {
  ...
  expiresAt: string;
  customerId?: string;
  /** Customer's email on the Polar account, fetched server-side
   *  via findCustomerById. Worker 1.8.0+. */
  email?: string;
  /** License tier - derived from the customer's Polar benefit
   *  grants. Defaults to `"v1"`; `"v2"` arrives once V2_BENEFIT_ID
   *  is set in the Worker env and the customer has a v2 grant. */
  tier?: "v1" | "v2";
}
...
export class LicenseClient {
  ...
  async startTrial(deviceId: string, email?: string): Promise<TrialResponse> {
    // Worker 1.6.0+: email is optional. When absent the Worker
    // synthesizes `trial-<deviceShort>@buttereditor.com` so Polar
    // still has a customer record to attach the license key to
    // the customer-facing flow stays one-tap (no email field).
