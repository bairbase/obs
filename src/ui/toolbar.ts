...
const BUTTONS = {
  ...
  block: [
    ...
    { id: "task-list",     icon: "list-checks",   label: "Task list",     kind: "insert" as const },
...
export function execListCmd(btn: BtnDef, schema: Schema, view: EditorView) {
  // Flat-list schema (PMX 0.18.37+): the toolbar button's `nodeName`
  // ("bullet_list" / "ordered_list") is now a vestigial label; what
  // we actually do is insert / convert a `list_item` with the right
  // `kind`. Maps the legacy node name back to the kind.
  const kind: "bullet" | "ordered" =
    btn.nodeName === "ordered_list" ? "ordered" : "bullet";
...
function renderRegularButton(id: string, ctx: RenderCtx): HTMLElement | null {
  ...
  if (def.nodeName && !ctx.schema.nodes[def.nodeName]) return null;
...
export function createToolbar(
  ...
  const renderDesktop = () => {
    ...
    dom.setAttribute("aria-label", "Butter formatting toolbar");
