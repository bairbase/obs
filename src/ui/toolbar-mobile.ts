...
function openMobileInsertImageDialog(
  ...
  const modal = new (class extends Modal {
    onOpen() {
      ...
      // eslint-disable-next-line obsidianmd/ui/sentence-case -- URL scheme `https://` is lowercase by convention; placeholder shows the literal syntax users type.
...
function renderButterMain(
  ...
  installOverscrollRubberBand(list);
  // Auto-append the `+` insert button at the end of the main row
  // unless the layout includes an explicit overflow placeholder
  // (in which case the user wanted a custom slot for it).
  const hasExplicitOverflow = layout.some((i) => i.type === "overflow");
  if (!hasExplicitOverflow) {
