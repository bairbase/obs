...
export default class ButterEditorPlugin extends Plugin {
  ...
  public get licenseStatus(): "valid" | "trial" | "expired" | "unlicensed" | "unknown" {
    return "valid";
  }
  ...
  private async _refreshLicenseStatusInner(_force = false): Promise<void> {
    return;
  }
  ...
  async resumeTrialActivation(): Promise<void> {
    if (!this.settings.pendingTrialActivation) return;
